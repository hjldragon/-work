...
class OrderEntry
{
    ...
    public $delete            = null;    // 0:未删除; 1:已删除 //顾客是否删除订单(但商户端依然会显示)
...
//            ]);
    ...
    public function GetOrderAllList($filter=null,$sortby=[])//,&$price_list=null)
    {
        ...
        if (null != $filter)
        {
            ...
            $kitchen_status = $filter['kitchen_status'];
            if (null !== $kitchen_status)
            {
                $cond['kitchen_status'] = (int)$kitchen_status;
            }
            ...
            // $order_begin_time = $filter['order_begin_time'];
            // $order_end_time   = $filter['order_end_time'];
            // if (!empty($order_begin_time) || !empty($order_begin_time))
            // {
            //     $cond['order_time'] = [];
            //     if($order_begin_time)
            //     {
            //         $cond['order_time']['$gte'] = (int)$order_begin_time;
            //     }
            //     if($order_end_time)
            //     {
            //         $cond['order_time']['$lte'] = (int)$order_end_time;
            //     }
            // }
...
//        }
    ...
    public function GetOrderStat($filter=null, $field=[], $sortby=[])
    {
        ...
        if(null != $filter)
        {
            ...
            if(!empty($order_status))
            {
                $cond["order_status"] = ['$in' => $order_status];
            }
        }
        if(empty($sortby))
        {
