...
// 打印机规格
var PrinterSize = new function()
{
    this.SPEC_80MM = 1;      // 80mm
    this.SPEC_58MM = 2;      // 58mm
    this.code = {
        1 : "80mm",
        2 : "58mm"
    };
...
// 打印机类别
var PrinterCategory = new function()
{
    this.PRINT_STOP          = 0;      // 不启用
    this.PRINT_ALL           = 1;      // 全单打印（适用于给客人的小票）
    this.PRINT_FOODNAME      = 2;      // 只打印菜名（适用于后厨打印等）
    this.PRINT_SPECIFY       = 3;      // 只打印特定菜名（如酒水、凉菜等指定餐品类型）
    this.PRINT_SPECIFY_NOCUT = 4;      // 只打印特定菜名（如酒水、凉菜等指定餐品类型），不切
...
// 员工职务
var EmployeeDuty = new function()
{
    this.UNKNOWN         = 0;   // 待定
    this.SYS_SHOP_ADMIN  = 1;   // 店铺系统管理员
    this.BOSS            = 2;   // 老板
    this.GENERAL_MANAGER = 3;   // 总经理
    this.SHOP_MANAGER    = 4;   // 店长
    this.FOREMAN         = 5;   // 领班
    this.WAITER          = 6;   // 服务员
    ...
    {
        code = parseInt(code||0);
        return this.n2str[code]||'未知[' + code + ']';
    }
...
// 员工权限
var EmployeePermission = new function()
{
    this.HasOrderRead = function(permission)
    {
        permission = permission||{};
        return !!permission["order_read"];
    }
    this.HasOrderWrite = function(permission)
    {
        permission = permission||{};
    ...
    //
    this.HasFoodRead = function(permission)
    {
        permission = permission||{};
        return !!permission["food_read"];
    }
    this.HasFoodWrite = function(permission)
    {
        permission = permission||{};
    ...
    //
    this.HasReportWrite = function(permission)
    {
        permission = permission||{};
...
// 店铺是否暂停
var ShopIsSuspend = new function()
{
    this.NO            = 0;    // 正常使用
    this.BY_SYS_ADMIN  = 1;    // 被系统管理员暂停
    this.BY_SHOP_ADMIN = 2;    // 被店铺管理员暂停
    ...
    this.IsSuspend = function(suspend)
    {
        suspend = parseInt(suspend||0);
        return this.BY_SYS_ADMIN == suspend || this.BY_SHOP_ADMIN == suspend;
    }
}

var FoodIsSaleOff = new function()
{
    this.YES = 1;  // 已下架
    this.NO  = 0;  // 正常
    this.n2str = {
