...
function GetPrinterOrderInfo(&$resp)
{
    ...
    if(!$employee_info)
    {
        $employee_info = '老板'; //<<<<<<<<<存在空数据？？原因还在查找中。。。
    }
   ...
   //操作信息
    ...
    $orderInfo['operUser']    = $employee_info->real_name;
    $orderInfo['waiter']      = '小明'; //<<<<<<<<<写死的数据
    ...
    $productInfo['totalPrice']   = $total_price;
    $productInfo['sq_code_url']  = '扫码自助开发票功能即将上线，敬请期待…';//<<<<<<<写死的数据
    $productInfo['mark']         = $info->order_remark;
    ...
    if($info->pay_status == 2)
    {
        ...
        $pay_info['need_money_extend'] = [$need];
        $pay_info['real_money']        = $info->paid_price;
        $pay_info['real_money_extend'] = [$real];
        //$pay_info['sq_code_url']       = 'http://shop.jzzwlcm.com/php/img_get.php?img=1&imgname=24b43aaa4b09b5a165d3b5bf78c761ed.jpg';
        $pay_info['sq_code_url']       = '扫码自助开发票功能即将上线，敬请期待…';
        $pay_info['mark']              = $info->order_remark;
    }
    if($info->order_status == 3)
    {
        $real['name']  = OrderPayWay::$pay_type[$info->pay_way];
        $real['price'] = $info->order_payable;
        $back_pay_info['real_money']        = $info->order_fee;
        ...
        $back_pay_info['mark']              = $info->order_remark;
    }
    if($info->is_advance == 1)
    {
        ...
        $pre_pay_info['mark']              = $info->order_remark;
    }
    $resp = (object)array(
        'orderInfo'    => $orderInfo,
