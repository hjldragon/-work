...
class AGEmployeeEntry
{
    ...
    public $ag_role_id       = null;     // 代理商角色id
    ...
    public $employee_num     = null;     // 员工编号 //???产品定义的
    ...
    public function FromMgo($cursor)
    {
        ...
        $this->employee_num     = $cursor['employee_num'];
    }
...
class AGEmployee
{
    ...
    public function Save(&$info)
    {
        ...
        if(null !== $info->ag_role_id)
        {
            $set["ag_role_id"] = (string)$info->ag_role_id;
        }
        ...
        if(null !== $info->employee_num)
        {
            $set["employee_num"] = (string)$info->employee_num;
        }
        $value = array(
            '$set' => $set
        );
    ...
    //通过平台部门下所有员工
    public function GetPositionAgEmployee($ag_role_id)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());
    ...
    //通过部门ID获取所有代理商员工
    public function GetEmployeeByName($agent_id,$real_name)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());
    ...
    //所有销售人列表
    public function GetAgEmployeeByPsIdList($agent_id, $ag_role_id_list)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());

        foreach($ag_role_id_list as $i => &$item)
        {
            $item = (string)$item;
        }
    ...
    //通过姓名所有销售人列表
    public function GetAgEmployeeByNameList($ag_employee_name,$ag_role_id_list)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());
        ...
        $cond = [
            'real_name'     => new \MongoRegex("/$ag_employee_name/"),
            'delete'        => ['$ne'=>1],
            '$or'    => [
                ["is_admin"      => 1],
                ['ag_position_id'=> ['$in' => $ag_role_id_list]]
            ]
        ];
