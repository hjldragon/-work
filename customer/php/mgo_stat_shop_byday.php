...
class StatShopEntry
{
    ...
    public $qr_order_num    = 0;     // 扫码点餐次数
    public $consume_amount  = 0;     // 消费总额
    //public $guest_price     = 0;     // 客单价
    public $customer_num    = 0;     // 消费者数
    public $platform_id     = null;  // 平台ID
    public $new_cus_num     = 0;     // 新增客户数
    public $active_cus_num  = 0;     // 活跃者数
...
class StatShop
{
    ...
    public function ActiveCusNum($shop_id, $day, $active_cus_num=null, $agent_id)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());
        ...
        $value = array(
            '$set' => array(
                'lastmodtime'    => time(),
                'delete'         => 0,
                'active_cus_num' => (int)$active_cus_num
            ),
        );
