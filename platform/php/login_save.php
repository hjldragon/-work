...
function Login(&$resp)
{
    ...
    $userinfo = $user->QueryUser($phone, $phone, $password_md5, UserSrc::PLATFOR);
    //LogDebug($userinfo);
    if(null == $userinfo)
    {
    ...
    $plinfo  = $plmgo->QueryByUserId($userinfo->userid);
    ...
    {   
        ...
        //拥有权限
        $ps_info        = $position->GetPositionById($plinfo->pl_position_id);
        $permission_arr = PermissionSplit($ps_info->pl_position_permission);
        $plinfo->permission = $permission_arr;
...
function GetCoke(&$resp){
    ...
    // 查找手机号码
    $ret = $mgo->QueryByPhone($phone,UserSrc::PLATFOR);
    //LogDebug($ret);
    switch ($name)
    {
        case 'forgetPwd':
...
function UserNewPasswd(&$resp)
{
    ...
    $phone      = $_['phone'];
    $mgo        = new \DaoMongodb\User;
    $user = $mgo->QueryByPhone($phone,UserSrc::PLATFOR);
