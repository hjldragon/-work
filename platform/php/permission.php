...
class Permission {
    ...
    //平台登陆员工一般页面权限检查
    public static function PagePlCheck()
    {
    ...
    public static function PlPermissionCheck()
    {
        ...
        if($platformer_info->is_admin != 1)
        {
            ...
            if(($position->pl_position_permission & \PlPosition::ALL) == 0 || ($position->pl_position_permission & \PlPosition::PLATFORMER) == 0)
            {
                    LogErr("this platformer permission is not enough");
                    return errcode::USER_PERMISSION_ERR;
            }
