...
class OrderEntry
{
    ...
    public $order_from        = null;    // 订单来源(1:门店前台(收银机), 2:手持设备(微信点餐), 3:扫码自助(自助点餐机),4:pad)
    ...
    public $selfhelp_id       = null;    // 来源自主点餐机的唯一识别id
...
//            ]);
    ...
    public function GetOrderAllList($filter=null)
    {
        ...
        if (null != $filter)
        {
            ...
            if (null !== $dine_way)
            {
                ...
                if($dine_way == \SALEWAY::PACK)
                {
                    $cond['$or'] = [
                        ['food_list.is_pack' => 1],
                        ['dine_way'          =>(int)$dine_way]
                    ];
                }else{
                    $cond['dine_way']        = (int)$dine_way;
                }
        ...
        if (null !== $plate && null === $seat_id_list)
        {
            $cond['plate'] = new \MongoRegex("/$plate/");
        }
    ...
    //改变订单中的菜品数据状态
    public function BatchFoodStatus($order_id_list, $id_list, $made_status)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());
...
//        }
        $cond = array(
            ...
            'food_list.id' => $id_list
        );

        $value = array(
            '$set'=>array(
                'food_list.$.made_status' => (int)$made_status,
                'lastmodtime'             => time(),
            )
        );
        ...
        try
        {
            $ret = $table->update($cond, $value, ['safe'=>false, 'upsert'=>true, 'multi' => true]);
    ...
    //改变订单中的菜品数据状态
    public function OrderUrge($order_id_list, $is_urge)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());
        ...
        $cond = array(
            'order_id'     => array('$in' => $order_id_list),
        );
    ...
    public function OrderInfoUrge($order_id, $is_urge)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());
        $cond = array(
