...
class Cfg
{
    ...
    private function Init()
    {
        ...
        // 日志配置
        $path = Util::EmptyToDefault($cfg["log.path"], "/log/kitchen.sailing.com/log.txt");
        if(file_exists($path))
        {
...
eof;
        ...
        // 点餐页面地址
        $this->menu_url       = Util::EmptyToDefault($cfg["menu_url"], "http://customer.jzzwlcm.com/index.php");
        $this->food_url       = Util::EmptyToDefault($cfg["food_url"], "http://customer.jzzwlcm.com/static/share/index.html");
        //$this->food_url     = Util::EmptyToDefault($cfg["food_url"], "http://customer.xinchihuo.com.cn/#/shareFoodDetail");
        $this->login_url      = Util::EmptyToDefault($cfg["login_url"], "http://wx.jzzwlcm.com/wx_login.php");
        $this->jump_url       = Util::EmptyToDefault($cfg["jump_url"], "http://wx.jzzwlcm.com/jumpto.php");
        $this->binding_url    = Util::EmptyToDefault($cfg["binding_url"], "http://wx.jzzwlcm.com/wx_binding.php");
        $this->api_domain     = Util::EmptyToDefault($cfg["api_domain"], "http://api.jzzwlcm.com");
        // 店铺地址
        $this->shop_url_addr  = Util::EmptyToDefault($cfg["shop_url_addr"], "http://shop.jzzwlcm.com");
        // 主域名
        $this->main_domain  = Util::EmptyToDefault($cfg["main_domain"], "jzzwlcm.com");
    ...
    // 取字体文件
    public function GetFontFile($type=null)
    {
        if(null == $type)
        {
        ...
        {
            $path = "{$this->data_basedir}/font/msyhbd.ttf";
        }
        Util::DirCheck(dirname($path));
        return $path;
    }
