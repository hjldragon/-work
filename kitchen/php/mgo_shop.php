...
class ShopEntry
{
    ...
    public $refresh_time      = null;               // 刷新时间(后厨通)
    public $is_show_wait_time = null;               // 是否显示等待时间(后厨通传菜模块)（1:是，0否,）
    public $is_change_urge    = null;               // 超时菜品是否自动发布催菜请求(后厨通配菜模块)（1:是，0否,）
    public $is_over_time      = null;               // 是否启用超时(1:是，0否)//用于设置菜品超时的时候发送超时单并变成催单单通知
    public $kitchen_seat_list = null;               // 后厨系统的餐桌区域优先显示设置
    public $order_clear_time  = null;               // 系统数据清楚时间<<<<<<<默认24小时
    function __construct($cursor = null)
    {
    ...
    public function FromMgo($cursor)
    {
        ...
        $this->refresh_time        = $cursor['refresh_time'];
        $this->is_show_wait_time   = $cursor['is_show_wait_time'];
        $this->is_change_urge      = $cursor['is_change_urge'];
        $this->is_over_time        = $cursor['is_over_time'];
        $this->kitchen_seat_list   = $cursor['kitchen_seat_list'];
        $this->order_clear_time    = $cursor['order_clear_time'];
...
//                $set["weixin_pay_set"] = $p;
        ...
        if (null !== $info->shop_bs_time){
            $set["shop_bs_time"] = (int)$info->shop_bs_time;
        }
        ...
        if (null !== $info->refresh_time){
            $set["refresh_time"] = (int)$info->refresh_time;
        }
        if (null !== $info->is_show_wait_time){
            $set["is_show_wait_time"] = (int)$info->is_show_wait_time;
        }
        if (null !== $info->is_change_urge){
            $set["is_change_urge"] = (int)$info->is_change_urge;
        }
        if (null !== $info->is_over_time){
            $set["is_over_time"] = (int)$info->is_over_time;
        }
        if (null !== $info->kitchen_seat_list){
            $set["kitchen_seat_list"] = $info->kitchen_seat_list;
        }
        if (null !== $info->order_clear_time){
            $set["order_clear_time"] =(int)$info->order_clear_time;
        }
        $value = array(
            '$set' => $set
        );
