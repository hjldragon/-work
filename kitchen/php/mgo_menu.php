...
class MenuInfoEntry
{
    ...
    public $overtime            = null;             // 超时时间
    public $over_set_time       = null;             // 创建超时时间时间
    ...
    {   
        ...
        $this->overtime            = $cursor['overtime'];
        $this->over_set_time       = $cursor['over_set_time'];
    }
...
class MenuInfo
{
    ...
    public function Save(&$info)
    {
        ...
        if (null !== $info->overtime) {
            $set["overtime"] = (int)$info->overtime;
        }
        if (null !== $info->over_set_time) {
            $set["over_set_time"] = (int)$info->over_set_time;
        }
    ...
    public function GetFoodinfoByCate($shop_id, $category_id)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());
    ...
    public function SetOverTime($food_id, $overtime)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());
...
//        }
    ...
    //获取该店铺下面所有餐品列表
    public function GetFoodAllList($shop_id, $filter = null)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());
        ...
        if (null != $filter) {
            ...
            {
                  $cond['$or'] = [
                        ['food_name' => new \MongoRegex("/$food_name/")],
