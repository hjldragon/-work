...
function SaveOrderInfo(&$resp)
{
    ...
    if($customer_phone){
        ...
        $result     = PageUtil::VerifyPhoneCode($token, $customer_phone, $phone_code);
        //验证手机结果
        if($result != 0){
            LogDebug($result);
            return $result;
    ...
    if($pay_way == PayWay::EATAFTER && $shopinfo->meal_after == ISMEALAFTER::NO)
    {
        LogErr("pay fail , shop don't can eat after");
        return errcode::NO_EAT_AFTER;
    }
    if(!$order_id){
        //非新建必须有的数据
    ...
    if($food_list){
        ...
        foreach ($food_list as $i => &$item) {
           ...
           //$price_one  = GetFoodPrice($item, $customer_id, $price);//菜品单价
            ...
            $food_list_all->weight           = $item->spec_type;
            if($item->pack_num)
            {
...
/*        if(IsInvoice::NO != (int)$invoice_type && $invoice_id){
    ...
    else
    {
        ...
        $food_num_all = $entry->food_num_all;
        $seat_price   = $entry->seat_price;
    }
    ...
    $entry->is_confirm        = 0;
    $entry->order_fee         = round($order_fee,2);
    $entry->order_payable     = round($order_fee,2);//应付价格=菜品总价+餐位费-减免费 order_waiver_fee可能由服务员修改（与客人端无关
