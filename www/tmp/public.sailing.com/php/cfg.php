...
class Pubcfg
{
    ...
    static private $module = "public"; // 公共端
    public $db   = null;
    public $log  = null;
    public $rsa  = null;
    public $img  = null;
    ...
    private function Init()
    {
        ...
        // 日志配置
        $this->path = Util::EmptyToDefault($cfg["log.path"], "/public.xinchihuo.com.cn/log.txt");
        if(file_exists($this->path))
        {
            $this->path = realpath($this->path);    // 注：若文件不存在时，realpath()调用返回空
        }
        ...
        $this->invoice = (object)[
            "appid"        => Util::EmptyToDefault($cfg['invoice.appid'], "wxe6ee6bc6898df9df"),
            "appkey"       => Util::EmptyToDefault($cfg['invoice.appkey'], "ofVDMK1MOKWUEum5Z6sCAWeUtQ9ynR8m"),
            "appsecret"    => Util::EmptyToDefault($cfg['invoice.appsecret'], "GPL83NTjsRCU79FmsSEwhftRWltCZN40Q5kuCrDYBkZU1ave"),
            "taxpayer_num" => Util::EmptyToDefault($cfg['invoice.taxpayer_num'], "91440300MA5ENTGUOH"),
            "callback_url" => Util::EmptyToDefault($cfg['invoice.callback_url'], "http://mart.xinchihuo.com/php/invoice_notify.php"),
            "card_id"      => Util::EmptyToDefault($cfg['invoice.card_id'], "pF_Yo0SzdbFoqSBFDZ4BlvugB488"),
            "tax_rate"     => Util::EmptyToDefault($cfg['invoice.tax_rate'], "0.03")
        ];
