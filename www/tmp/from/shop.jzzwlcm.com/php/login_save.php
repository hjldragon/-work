...
function LoginWxApp(&$resp)
{
    ...
    $weixin = \Cache\Weixin::Get($openid, $src, $srctype);
...
function LoginMp(&$resp)
{
    ...
    $employee_one = $employee->QueryByUserId($shop_id,$userinfo->userid);
    ...
    {
        LogErr("Login err");
        return errcode::USER_LOGINED_EXIST;
    }
    ...
    {
        $weixin   = new \DaoMongodb\Weixin;
        $wx_user = $weixin->UserIdAndSrctype($userinfo->userid, WxSrctype::PC, Src::SHOP);
        ...
        if($weixin_info->userid && $weixin_info->userid != $userinfo->userid)
        {
            LogErr("WeixinUser err");
        ...
        $wx_entry->userid      = $userinfo->userid;
        ...
        $user_entry->userid    = $userinfo->userid;
        $user_entry->is_weixin = 1;
        $save = $user->Save($user_entry);
        if(0 != $save)
        {
            LogErr("UserSave err");
...
else if(isset($_['login_mp']))
{
    $ret = LoginMp($resp);
}
else if(isset($_['logout_mp']))
{
    $ret = MpLogout($resp);
}
else
