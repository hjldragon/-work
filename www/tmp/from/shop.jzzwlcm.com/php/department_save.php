...
function SaveDepartment(&$resp)
{
    ...
    if (!$department_id)
    {
        ShopPermissionCheck::PageCheck(ShopPermissionCode::ADD_DEPARTMENT);
        $department_id = \DaoRedis\Id::GenDepartmentId();
        LogDebug($department_id);
    }else{
        ShopPermissionCheck::PageCheck(ShopPermissionCode::EDIT_DEPARTMENT);
    }
...
{
    ShopPermissionCheck::PageCheck(ShopPermissionCode::DEL_DEPARTMENT);
    $_ = $GLOBALS["_"];
    if (!$_)
    {
        LogErr("param err");
