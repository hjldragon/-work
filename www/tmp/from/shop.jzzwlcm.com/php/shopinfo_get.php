...
//    $shopinfo['shop_id']     = $info->shop_id;
    ...
    $shopinfo['city']            = $info->city;
    $shopinfo['area']            = $info->area;
    $shopinfo['business_status'] = $info->business_status;
...
{
    ShopPermissionCheck::PageCheck(ShopPermissionCode::SEE_SHOP_SET);
    $_ = $GLOBALS["_"];
    if(!$_)
    {
        LogErr("param err");
...
function GetShopBusinessInfo(&$resp)
{
    ...
    if (!$shop_id)
    {
     ...
     $bus   = new Mgo\Business;
     $info  = $bus->GetByShopId($shop_id);
     if(!$info->shop_id)
     {
         LogDebug('no business');
         return errcode::PARAM_ERR;
     }
    $resp = (object)[
        'business'   => $info,
...
{
    ShopPermissionCheck::PageCheck(ShopPermissionCode::SEE_SHOUYIN_SET);
    $_ = $GLOBALS["_"];
    if (!$_)
    {
        LogErr("param err");
    ...
    $shopinfo['mailing_type']  = $info->collection_set->mailing_type;
      //var_dump($info->weixin_pay_set->pay_way);
    $resp = (object)[
        'collection_set' => $shopinfo,
        'weixin_set_way' => $info->weixin_pay_set->pay_way,
        'alipay_set_way' => $info->alipay_set->pay_way
    ];
    //LogDebug($resp);
    LogInfo("--ok--");
    return 0;
...
{
    ShopPermissionCheck::PageCheck(ShopPermissionCode::SEE_ALIPAY_SET);
    $_ = $GLOBALS["_"];
    if (!$_) {
        LogErr("param err");
        return errcode::PARAM_ERR;
...
{
    ShopPermissionCheck::PageCheck(ShopPermissionCode::SEE_WEIXIN_SET);
    $_ = $GLOBALS["_"];
    if (!$_) {
        LogErr("param err");
        return errcode::PARAM_ERR;
