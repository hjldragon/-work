...
// class Cfg extends Pubcfg
// {
//     static private $module_two = 'shop';
    ...
    //     $ret = $dao->QueryByModule(self::$module_two);
    //     foreach($ret as $item)
    //     {
...
class Cfg
{
    ...
    static private $module = "shop"; // 店铺端
    public $db   = null;
    public $log  = null;
    public $rsa  = null;
    public $img  = null;
    ...
    private function Init()
    {
        ...
        // 日志配置
        $path = Util::EmptyToDefault($cfg["log.path"], "/log/shop.xinchihuo.com.cn/log.txt");
        if(file_exists($path))
        {
            $path = realpath($path);    // 注：若文件不存在时，realpath()调用返回空
        }
