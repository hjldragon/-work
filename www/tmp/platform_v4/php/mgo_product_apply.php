<?php
/*
 * [Rocky 2017-04-26 18:00:57]
 * 代理商操作类
 */
declare(encoding='UTF-8');
namespace DaoMongodb;
require_once("db_pool.php");
require_once("const.php");

class ProductApplyEntry
{
    public $apply_id        = null;           // 申请id
    public $shop_name       = null;           // 商户名称
    public $apply_name      = null;           // 申请联系人
    public $telephone       = null;           // 联系电话
    public $address         = null;           // 地址
    public $city            = null;           // 城市
    public $area            = null;           // 区域
    public $province        = null;           // 省份
    public $email           = null;           // 邮箱
    public $product_status  = null;           // 试用产品(1.掌柜通（手机app),2.微信服务号,3.收银机,4.自助点餐机,5.扫码点餐收银系统,6.pad点餐收银系统)
    public $apply_time      = null;           // 申请时间
    public $apply_status    = null;           // 申请状态(0.未处理,1.已处理)
    public $pl_employee_id  = null;           // 处理人id
    public $deal_message    = null;           // 处理信息
    public $deal_time       = null;           // 更进时间
    public $purpose         = null;           // 意向程度(1.高,2.中,3.低)
    public $lastmodtime     = null;           // 最后修改的时间
    public $delete          = null;           // 是否删除(0:未删除; 1:已删除)





    function __construct($cursor = null)
    {
        $this->FromMgo($cursor);
    }

    // mongodb查询结果转为结构体
    public function FromMgo($cursor)
    {
        if (!$cursor) {
            return;
        }
        $this->apply_id        = $cursor['apply_id'];
        $this->shop_name       = $cursor['shop_name'];
        $this->apply_name      = $cursor['apply_name'];
        $this->telephone       = $cursor['telephone'];
        $this->address         = $cursor['address'];
        $this->province        = $cursor['province'];
        $this->city            = $cursor['city'];
        $this->area            = $cursor['area'];
        $this->email           = $cursor['email'];
        $this->product_status  = $cursor['product_status'];
        $this->apply_time      = $cursor['apply_time'];
        $this->apply_status    = $cursor['apply_status'];
        $this->pl_employee_id  = $cursor['pl_employee_id'];
        $this->deal_message    = $cursor['deal_message'];
        $this->deal_time       = $cursor['deal_time'];
        $this->purpose         = $cursor['purpose'];
        $this->lastmodtime     = $cursor['lastmodtime'];
        $this->delete          = $cursor['delete'];
    }

    public static function ToList($cursor)
    {
        $list = array();
        foreach ($cursor as $item) {
            $entry = new ProductApplyEntry($item);
            array_push($list, $entry);
        }
        return $list;
    }

}

class ProductApply
{
    private function Tablename()
    {
        return 'product_apply';
    }

    public function Save(&$info)
    {
        if (!$info->apply_id)
        {
            LogErr("param err:" . json_encode($info));
            return \errcode::PARAM_ERR;
        }
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());

        $cond = array(
            'apply_id' => (string)$info->apply_id
        );

        $set = array(
            "apply_id" => (string)$info->apply_id,
            "lastmodtime" => time()
        );

        if (null !== $info->shop_name) {
            $set["shop_name"] = (string)$info->shop_name;
        }
        if (null !== $info->apply_name) {
            $set["apply_name"] = (string)$info->apply_name;
        }
        if (null !== $info->telephone) {
            $set["telephone"] = (string)$info->telephone;
        }
        if (null !== $info->address) {
            $set["address"] = (string)$info->address;
        }
        if (null !== $info->province) {
            $set["province"] = (string)$info->province;
        }
        if (null !== $info->city) {
            $set["city"] = (string)$info->city;
        }
        if (null !== $info->area) {
            $set["area"] = (string)$info->area;
        }
        if (null !== $info->email) {
            $set["email"] = (string)$info->email;
        }
        if (null !== $info->product_status) {
            $set["product_status"] = $info->product_status;
        }
        if (null !== $info->apply_status) {
            $set["apply_status"] = (int)$info->apply_status;
        }
        if (null !== $info->apply_time) {
            $set["apply_time"] = (int)$info->apply_time;
        }
        if (null !== $info->pl_employee_id) {
            $set["pl_employee_id"] = (string)$info->pl_employee_id;
        }
        if (null !== $info->deal_message) {
            $set["deal_message"] = (string)$info->deal_message;
        }
        if (null !== $info->deal_time) {
            $set["deal_time"] = (int)$info->deal_time;
        }
        if (null !== $info->purpose) {
            $set["purpose"] = (int)$info->purpose;
        }
        if (null !== $info->delete) {
            $set["delete"] = (int)$info->delete;
        }
        $value = array(
            '$set' => $set
        );
        try {
            $ret = $table->update($cond, $value, ['safe' => true, 'upsert' => true]);
            LogDebug("ret:" . $ret['ok']);
        } catch (\MongoCursorException $e) {

            LogErr($e->getMessage());
            return \errcode::DB_OPR_ERR;
        }
        return 0;
    }

    // 返回 agentEntry
    public function GetProductApplyList($filter=null, $page_size, $page_no, $sortby = [], &$total=null)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());

        $cond = [
            'delete'  => ['$ne'=>1]
        ];
        //搜索功能
        if(null != $filter)
        {
            if(null != $filter['apply_status'])
            {   
                $apply_status = (int)$filter['apply_status'];
                if (0 == $apply_status) {
                    $cond['apply_status'] = ['$ne' => 1]; // ==0 或 ==null
                } else {
                    $cond['apply_status'] = 1;
                }
            }
            $province   = $filter['province'];
            if(!empty($province))
            {
                $cond['province'] = $province;
            }
            $area   = $filter['area'];
            if(!empty($area))
            {
                $cond['area'] = $area;
            }
            $city   = $filter['city'];
            if(!empty($city))
            {
                $cond['city'] = $city;
            }
            $telephone   = $filter['telephone'];
            if(!empty($telephone))
            {
                $cond['telephone'] = $telephone;
            }
            $begin_time = $filter['begin_time'];
            $end_time   = $filter['end_time'];
            if(!empty($begin_time))
            {
                $cond['apply_time'] = [
                    '$gte' => (int)$begin_time,
                    '$lte' => (int)$end_time
                ];
            }
        }
        if(empty($sortby)){
            $sortby['ctime'] = -1;
        }
        LogDebug($cond);
        $cursor = $table->find($cond, ["_id"=>0])->sort($sortby)->skip(($page_no-1)*$page_size)->limit($page_size);
        if(null !== $total){
            $total = $table->count($cond);
        }
        return ProductApplyEntry::ToList($cursor);
    }

    public function BatchChangeById($apply_id_list, $type=null)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());

        foreach($apply_id_list as $i => &$id)
        {
            $id = (string)$id;
        }

        $cond = array(
            'apply_id' => array('$in' => $apply_id_list)
        );
         if(1 == $type)
         {
             $set = array(
                 "lastmodtime" => time(),
                 "delete"      => 1,
             );
         }
         if(2 == $type)
         {
             $set = array(
                 "lastmodtime"  => time(),
                 "apply_status" => 1,
             );
         }
        $value = array(
            '$set' => $set
        );
        try
        {
            $ret = $table->update($cond, $value, ['safe'=>true, 'upsert'=>true, 'multiple' => true]);
            LogDebug("ret:" . $ret['ok']);
        }
        catch(\MongoCursorException $e)
        {
            LogErr($e->getMessage());
            return \errcode::DB_OPR_ERR;
        }
        return 0;
    }

    public function QueryByParam($telephone, $shop_name)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());

        $cond = array(
            'telephone'      => (string)$telephone,
            'shop_name'      => (string)$shop_name,
            'delete'         => 0,
        );

        $ret = $table->find($cond);
        return ProductApplyEntry::ToList($ret);
    }

    public function GetApplyIdList($filter=null, $field=[], $sortby=[])
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());

        $cond = [
            'delete'      => ['$ne'=>1],
        ];
        if(null != $filter)
        {
            $apply_id_list = $filter['apply_id_list'];
            if(!empty($apply_id_list))
            {
                foreach($apply_id_list as $i => &$item)
                {
                    $item = (string)$item;
                }
                $cond["apply_id"] = ['$in' => $apply_id_list];
            }
        }
        if(empty($sortby))
        {
            $sortby['_id'] = -1;
        }
        $field["_id"] = 0;

        $cursor = $table->find($cond, $field)->sort($sortby);
        return ProductApplyEntry::ToList($cursor);
    }

    public function GetInfoById($apply_id)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());

        $cond = [
            'apply_id' => (string)$apply_id,
            'delete'   => ['$ne'=>1]
        ];
        $cursor = $table->findOne($cond);
        return new ProductApplyEntry($cursor);
    }
}


?>
