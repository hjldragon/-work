...
function pay()
{
    ...
    if($shopinfo->auto_order == AutoOrder::Yes) //用于判断店铺端设置是否支付自动下单,因为没设置的话需要Pad来确认
    {
        ...
        // 更新餐品日销售量
        // 增加餐品售出数<<<<<<<<<2018.7.11,产品和测试说与自动下单没关系了
    ...
    //发送短信
    $order  = $mgo->GetOrderById($order_info->order_id);
    PageUtil::PayOrderGetMessage($order, $shopinfo);
    //如果属于直接确认单，就保存统计数据
    //保存统计数据
    PageUtil::PayOrderBoard($order, $shopinfo, $agent_info, $data['total_amount']);
    // 更新缓存
    ...
    if($token)
    {
        ...
        $ret_json = PageUtil::NotifyAlipayPay($order_info->order_id, $data['total_amount'] ,$token);
        $ret_json_obj = json_decode($ret_json);
        if(0 != $ret_json_obj->ret)
        {
