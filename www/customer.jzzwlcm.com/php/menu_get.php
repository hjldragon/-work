...
function GetRate($food_id)
{
    ...
    $data = [];
    $mgo     = new \DaoMongodb\Evaluation;
    $all     = $mgo->GetFoodAllCount($food_id, null, $all_stat);
    $is_good = $mgo->GetFoodAllCount($food_id, 1);
    if($all > 0){
...
function GetFoodInfo(&$resp)
{
    ...
    // 好评率
    $good_rate = GetRate($food_id);
    $info->good_rate = $good_rate['good'];
    if($customer_id)
    {
        //是否点赞
...
function GetMenuInfo(&$resp)
{
    ...
    $shopinfo['province']          = $shop_info->province;
    $shopinfo['city']              = $shop_info->city;
    $shopinfo['area']              = $shop_info->area;
        ...
        foreach ($food_id_list as $food_value)
        {
            ...
            if($food_value->food_attach_list)
            {
                ...
                foreach ($food_value->food_attach_list as $spec_value)
                {
                    ...
                    $spec_one['list']  = $spec_value->spec_value;
            ...
            $good_rate = GetRate($food_value->food_id);
            $food_list['good_rate']     = $good_rate['good'];
            //餐品的好评率
...
function GetPraiseMenu(&$resp)
{
    ...
    foreach ($category_list as $i => &$category)
    {
        ...
        foreach ($food_id_list as $food_value)
        {
            ...
            $food_list['feature']       = $food_value->feature;
            $good_rate = GetRate($food_value->food_id);
            $food_list['good_star']     = $good_rate['star'];
            array_push($food_list_all, $food_list);
        }
        if(!$food_list_all)
        {
