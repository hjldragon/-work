...
class ShopEntry
{
    ...
    public $shop_model        = null;               // 营业模式 (1:中餐,2.西餐,3.外卖,4.火锅,5.夜宵,6.地方菜)
...
//                $set["alipay_set"] = $p;
    ...
    public function GetShopByAgentId($agent_id)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());
    ...
    public function GetShopList($filter = null, $page_size, $page_no, $sortby = [], &$total=null)
    {
        ...
        if (null != $filter) {
            ...
            if (!empty($shop_model)) {
                $cond['shop_model'] = (int)$shop_model;
            }
    ...
    public function GetByFromSalesman($from_salesman, $agent_id, &$total=null)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());

        $cond = array(
            'from_salesman' => (string)$from_salesman,
            'agent_id'      => (string)$agent_id,
            'delete'        => ['$ne' => 1]
        );
        $field["_id"] = 0;
