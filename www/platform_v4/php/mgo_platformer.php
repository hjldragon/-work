...
class PlatformerEntry
{
    ...
    public $platformer_num   = null;           // 员工编号 //???产品定义的
    public $spec_record      = null;           // 商品规格历史输入记录
    ...
    public function FromMgo($cursor)
    {
        ...
        $this->platformer_num   = $cursor['platformer_num'];
        $this->spec_record      = $cursor['spec_record'];
    }
...
class Platformer
{
    ...
    public function Save(&$info)
    {
        ...
        if (null !== $info->pl_role_id) {
            $set["pl_role_id"] = (string)$info->pl_role_id;
        }
        ...
        if (null !== $info->spec_record) {
            $set["spec_record"] = $info->spec_record;
        }
        if(null !== $info->is_freeze)
        {
            $set["is_freeze"] = (int)$info->is_freeze;
        }
        if(null !== $info->platformer_num)
        {
            $set["platformer_num"] = (string)$info->platformer_num;
        }
    ...
    public function QueryByUserId($userid, $platform_id)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());
    ...
    //查找职位下面是否有员工
    //通过角色所有员工
    public function GetPositionPlatformer($pl_role_id)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());
    ...
    public function QueryByPlName($pl_name)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());

        $cond = array(
            'pl_name' => (string)$pl_name,
            'delete'  => array('$ne'=>1)
        );
        $ret = $table->findOne($cond);
        return new PlatformerEntry($ret);
    ...
    //所有销售人列表
    public function GetByPsIdList($pl_role_id_list)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());

        foreach($pl_role_id_list as $i => &$item)
        {
            $item = (string)$item;
        }
