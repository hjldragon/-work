...
if($_REQUEST['send'])
{
    ...
    $ret_json = NotifyWxPay($order_id, $price, $token);
...
const vm = new Vue({
    ...
    mounted(){
        // let url = "ws://127.0.0.3:13010/websocket";      // 开发环境
        let url = "ws://srv.jzzwlcm.com:13010/websocket";   // 测试环境
        // let url = "ws://srv.xinchihuo.com.cn:13010/websocket"; // 正式环境
        window.WebSock.Init(url, this.token, this.key, ()=>{

            window.WebSock.Subscribe("once",
                {
                    topic : "alipay_notify@" + this.token,
                },
                (resp) => {
                    console.log(resp);
