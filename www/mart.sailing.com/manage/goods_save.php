...
function SaveGoodsInfo(&$resp)
{
    ...
    $first_sale_num  = $_["first_sale_num"];
    ...
    if($edit_type)//新增
    {
        if(!$cate_type || !$goods_name || !$goods_type || !$spec_type || !$freight_type)
        {
            LogErr("param err");
            return errcode::PARAM_ERR;
        }
    ...
    if($spec_price)
    {
        ...
        foreach ($spec_price as $item)
        {
            ...
            if(!$spec_id)
            {
                ...
                $entry_time = time();
                // $spec_info = $spec_mgo->GetSpecByName($goods_id, $item->spec_name, $item->package);
                // if($spec_info->spec_id)
                // {
    ...
    $entry->first_sale_num  = $first_sale_num;
