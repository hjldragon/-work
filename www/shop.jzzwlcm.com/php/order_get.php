...
function GetOrderAllList(&$resp)
{
    ...
    {
        $order_begin_time = 0; //默认时间戳开始时间
    }
    if (!$order_end_time && $order_begin_time)
    {
        $order_end_time = time(); //默认后面很长时间
    }
    if (!$begin_time && $end_time)
    {
        $begin_time = 0; //默认时间戳开始时间
    }
    if (!$end_time && $begin_time)
    {
        $end_time = time(); //默认后面很长时间
    }
//    LogDebug($_);
    switch ($sort_name) {
        case 'order_id':
...
//    }
    ...
    foreach ($order_list as &$v)
    {
        ...
        if($v->selfhelp_id && $srctype == NewSrcType::SHOUYINJI){
...
function GetOrderPending(&$resp)
{
    ...
    $order_status_list  = [NewOrderStatus::NOPAY,NewOrderStatus::PAY,NewOrderStatus::REFUNDING];
    $shop_id            = $_['shop_id'];
    $sort['order_time'] = -1;//<<<<排序
    ...
    foreach ($infos as $v)
    {
        ...
        if(($v->order_status == NewOrderStatus::PAY && $v->is_confirm == IsCoonfirm::NO)|| ($v->order_status == NewOrderStatus::NOPAY && $v->is_confirm == IsCoonfirm::NO))
        {
            $type = 0; //新订单
        }
        ...
        elseif($v->order_status == NewOrderStatus::NOPAY && $v->pay_status == PayStatus::NOPAY)
        {
            $type = 2; //下单未支付
        } else{
            continue;
