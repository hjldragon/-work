...
class MenuInfo
{
    ...
    public function GetSelfhelpFoodList($shop_id, $filter = null)
    {
        $db = \DbPool::GetMongoDb();
        $table = $db->selectCollection($this->Tablename());
        ...
        if (null != $filter) {
            $sale_way_list = $filter['sale_way_list'];
            if(!empty($sale_way_list))
            {
                foreach($sale_way_list as $i => &$item)
                {
                    $item = (int)$item;
                }
                $cond["sale_way"] = ['$in' => $sale_way_list];
            }
