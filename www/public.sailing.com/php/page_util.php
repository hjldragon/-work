...
// 使用签名加密形式发送
// $opt -- 可选设置项
// 注：当前代码和HttpPostJsonEncData()的处理过程相关，但返回的是对象（不是字符串）
//     以后使用此PostJsonEncData();
// -- Rocky 2019-01-14
static function PostJsonEncData($url, $data, $opt=[])
{
    $data = (object)$data;
    $opt = (object)$opt;
    ...
    $ret = self::HttpPostJsonData($url, $data, $timeout);
    if(!$ret)
    {
        return (object)[
            'ret' => errcode::SYS_ERR,
            'data' => (object)[
                'errmsg' => "PostJsonData err, url:[$url]",
            ]
        ];
    }
